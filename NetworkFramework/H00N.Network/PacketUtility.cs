...
namespace H00N.Network
{
    public class PacketUtility
    {
        ...
        public static ushort ReadListData<T>(ArraySegment<byte> buffer, int offset, out List<T> result) where T : DataPacket, new()
        {
            ...
            ushort listLength = BitConverter.ToUInt16(buffer.Array, buffer.Offset + offset);
        ...
        {
            ushort process = 0;
            process += AppendUShortData((ushort)data.Count, buffer, offset);

            for(int i = 0; i < data.Count; i++)
                process += AppendDataPacket<T>(data[i], buffer, offset + process);
